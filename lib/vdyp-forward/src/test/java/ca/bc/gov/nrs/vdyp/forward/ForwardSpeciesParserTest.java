...
class ForwardSpeciesParserTest {
	...
	@Test
	void testYABHInferredWhenTotalAgeGreaterThanYTBH() throws Exception {
		...
		var fileResolver = TestUtils.fileResolverContext(
				"test.dat",
				TestUtils.makeInputStream(
						"01002 S000002 00     1970 P 15 L  L  100.0     0.0     0.0     0.0 14.63 15.0  10.0  -9.0  6.30 0 14",
						"01002 S000001 00     1970"
				)
		);
		...
		// check for the passed in yabh to have been used as the height since the total age is less than the years to
		// breast height
		assertThat(genera.stream().findFirst().get().getSite().get().getYearsAtBreastHeight(), present(closeTo(3.7f)));
