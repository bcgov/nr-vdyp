...
class ForwardProcessingEngineTest {
	...
	class ProcessPolygon {
		...
		void setup() throws IOException, ResourceParseException, ValueParseException {
			...
			controlMap.put(
					ControlKey.VTROL.name(), new ForwardControlVariables(
							new Integer[] { -1, 1, 2, 3, 1, 1, 0, 0, 0, 0 }
					)
			);
			em = EasyMock.createControl();
			VdypOutputWriter output = new VdypOutputWriter(
					controlMap, OutputStream.nullOutputStream(), OutputStream.nullOutputStream(), OutputStream
							.nullOutputStream()
			) {
			...
			final var inputPoly = VdypPolygon.build(pb -> {
				...
				pb.addLayer(lb -> {
					...
					lb.addSpecies(sb -> {
						...
						sb.sp64DistributionSet(new Sp64DistributionSet(4, List.of())); // TODO this seems wrong, may need to fix parser
