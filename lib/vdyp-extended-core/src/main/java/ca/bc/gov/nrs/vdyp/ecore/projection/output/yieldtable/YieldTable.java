...
//		}
	 ...
	 *
	 * Obtains the density, height, basal area and diameter information for a specific species group at a specific layer
	 * at the age requested.
	 *
	 *
	 * @param polygon  The polygon for which you want the yields.
	 * @param layerID  The particular layer for which you want the layer information. If NULL or "", the last referenced
	 *                 layer will be used.
	 *
	 * @param sp0Name  The name of the SP0 species for which you want the growth information. If an SP64 is supplied, it
	 *                 will be converted into an appropriate SP0 code for look up.
	 *
	 * @param totalAge The total age of the stand for which you want the growth information.
	 *
	 *
	 * @returns LayerYields a record of the growth informatino indcating the growth data and wether the sp0 is the
	 *          dominant spieces for the requested year
	 *
	 *          Remarks -------
	 *
	 *          Note that certain layers may not have been processed. As a result, those layers may result in no growth
	 *          info despite the presence of info on other layers.
	 *
	 *          Note that different species may become the dominant species over the entire range of a projection.
	 *
	 *          2004/10/10 Implemented a rule to return projected site information if and only if input site information
	 *          appears in the SP0 group. See Cam's Oct. 10, 2004 e-mail.
	 *
	 *          2005/09/23: Now allow the retrieval of growth information for layers that were not processed.
	 *
	 *          2007/11/13: Based on direction from Sam Otukol, prevent projected (forward) heights from shrinking below
	 *          input height. Also, prevent projected (backwards) heights from exceeding input height. This check would
	 *          apply to the leading site species.
	 *
	 *          Also, fill in Diameter when computable from BA and TPH and not already being returned.
	 *
	 *          2008/04/16: When BA/TPH is not projected but available on input, copy it over as the projected BA/TPH.
	 *          Pro-rate by species percent on input.
	 *
	 *          2008/06/19: Relaxed the above constraint so that either of input BA and TPH are copied individually over
	 *          to the Projected BA/TPH.
	 *
	 *          2008/07/11: Further suppress this action if there is a VDYP7 Yield row predicted for the year in
	 *          question.
	 *
	 *          2008/07/21: Relax the constraint so that if the per hectare yields were suppressed (because of
	 *          IPSCB206), we will also want input BA/TPH copied forward.
	 *
	 *          2009/06/24: Added support for optionally turning the substitution of BA/TPH on or off.
	 *
	 */
	public LayerYields getProjectedLayerGroupGrowthInfo(
			Polygon polygon, Map<Integer, VdypPolygon> polygonProjectionsByYear, YieldTableRowContext rowContext,
			String layerID, String sp0Name, int totalAge
	) throws YieldTableGenerationException, StandYieldCalculationException {
		if (polygon == null) {
			throw new IllegalArgumentException("Polygon parameter not supplied.");
		}

		if (sp0Name == null) {
			throw new IllegalArgumentException("SP0 Name parameter not supplied.");
		}

		if ( (totalAge < 0) || (totalAge > 2000)) {
			throw new IllegalArgumentException("Total Age parameter out of range. Must be between 0 and 2000.");
		}

		// Check that the polygon state indicates that it has been projected.
		if (!rowContext.getPolygonProjectionState().polygonWasProjected()) {
			logger.error("ERROR: Polygon not in PROJECTED state.");
			throw new YieldTableGenerationException("ERROR: Polygon not in PROJECTED state.");
		}

		// Get the layer we are interested in.
		Layer layer;
		layer = polygon.getLayerById(layerID);
		if (layer == null) {
			logger.error("ERROR: Unable to locate requested layer: '{}'", layerID);

			throw new YieldTableGenerationException("ERROR: Unable to locate requested layer: '" + layerID + "'");
		}

		// Check that the species group requested is in the stand.
		Stand stand;
		...
		// this would be a large departure from the
		stand = layer.getSp0sByNameMap().getOrDefault(sp0Name, null);

		if (stand == null) {
			logger.error("ERROR: Unable to locate species '{}' in layer '{}'", sp0Name, layerID);
			throw new YieldTableGenerationException(
					"ERROR: Unable to locate species 'vdyp7Spcs' in layer '" + layerID + "'"
			);
		} else {
			logger.debug("Layer '{}' has stand information for SP0 '{}'", layerID, sp0Name);
		}

		/*
		 * Obtain the yield information for this SP0 at the requested age.
		 *
		 * Note that we have to convert the species group number into a 1 based index because index 0 is reserved for
		 * the layer summary information.
		 *
		 */

		LayerYields yields = obtainStandYield(rowContext, polygonProjectionsByYear, layer, stand, totalAge);

		/*
		 * Return the stats we are interested in.
		 *
		 */
		double updateSI = yields.siteIndex();
		double updateDominantHeight = yields.dominantHeight();
		double updateTPH = yields.treesPerHectare();
		double updateBasalArea = yields.basalArea125cm();
		double updateDiameter = yields.diameter();

		/*
		 * Oct. 10, 2004: Suppress site information if it does not appear on the input SP0
		 *
		 */

		Species standSpeciesGroup = stand.getSpeciesGroup();
		if (standSpeciesGroup.getSiteIndex() == null || standSpeciesGroup.getSiteIndex() < 0)
			updateSI = -9.0;
		if (standSpeciesGroup.getDominantHeight() == null || standSpeciesGroup.getDominantHeight() < 0)
			updateDominantHeight = -9.0;

		/*
		 * 2005/01/13: Projected Per Hectare Values are suppressed according to IPSCB206.
		 *
		 */
		...
		if (layer.getDoSuppressPerHAYields()) {
			...
			logger.debug("Per Hectare Projected Values were Suppressed.");
		}

		/*
		 * 2007/11/13: Projected Height (going forward) must never be less the reference height.
		 *
		 * Projected Height (going backwards) must never exceed the reference height.
		 *
		 * We don't compare against any other species but the leading species because secondary species are allowed to
		 * range ahead of or behind the lead species in height.
		 *
		 */
		int measuredYear = polygon.getReferenceYear();
		int projYear = yields.standYear();
		if ( (polygon.getReferenceYear() != -9) && (stand == leadSpcs) && // TODO do I need this?
				(updateDominantHeight >= 0.0)) {
			if (projYear >= measuredYear && updateDominantHeight < stand.getSpeciesGroup().getDominantHeight()) {
				updateDominantHeight = standSpeciesGroup.getDominantHeight();
			} else if (projYear < measuredYear && updateDominantHeight > standSpeciesGroup.getDominantHeight()) {
				updateDominantHeight = standSpeciesGroup.getDominantHeight();
			}
		}

		/*
		 * 2008/04/16: If projected BA/TPH were not supplied but are available on input, copy these values over to
		 * projected. This also implies that diameter should be computed as well.
		 *
		 * 2008/06/19: Relaxed the constraint so that if either of the values are there, each is copied out
		 * individually.
		 *
		 * 2008/07/02: Moved this logic so that it is always performed, whether or not there is a previous error. See
		 * Sam's e-mail dated: June 26, 2008
		 *
		 * 2008/07/11: Further suppress this action if there is a VDYP7 Yield row predicted for the year in question.
		 *
		 * 2008/07/21: Relax the constraint so that if the per hectare yields were suppressed (because of IPSCB206), we
		 * will also want input BA/TPH copied forward.
		 *
		 * 2009/06/24: Added support for optionally turning the substitution of BA/TPH on or off.
		 *
		 */

		if ( (!yields.bYieldsPredicted() || layer.getDoSuppressPerHAYields())
				&& (context.getParams().containsOption(ExecutionOption.DO_ALLOW_BA_AND_TPH_VALUE_SUBSTITUTION))
				&& updateBasalArea <= 0.0 && updateTPH <= 0.0) {
			boolean copiedBA = false;
			boolean copiedTPH = false;

			if (layer.getBasalArea() >= 0.0) {
				updateDiameter = -9.0;
				updateBasalArea = (layer.getBasalArea() * standSpeciesGroup.getSpeciesPercent() / 100.0);
				copiedBA = true;

				logger.debug(
						"Projected BA for SP0 '%s' not available. Copying Input BA %.3f protated (%.2f%%) to Projected value: %.3f",
						sp0Name, layer.getBasalArea(), standSpeciesGroup.getSpeciesPercent(), updateBasalArea
				);
			}

			if (layer.getTreesPerHectare() >= 0.0) {
				updateDiameter = -9.0;
				updateTPH = (layer.getTreesPerHectare() * standSpeciesGroup.getSpeciesPercent() / 100.0);
				copiedTPH = true;
				logger.debug(
						"Projected TPH for SP0 '{0}' not available. Copying Input TPH {1} protated {2} to Projected value: {3}",
						sp0Name, layer.getTreesPerHectare(), standSpeciesGroup.getSpeciesPercent(), updateTPH
				);
			}

			if (copiedBA || copiedTPH) {
				// TODO process the logic for the message to ensure BA, BA and TPH or TPH populates the info message
				// "Starting values for BA and TPH copied over for QA and alternative model used for SP0 '%s'",
				polygon.addMessage(
						new PolygonMessage.Builder().polygon(polygon).layer(layer)
								.details(
										ReturnCode.ERROR_LAYERNOTPROCESSED, MessageSeverityCode.INFORMATION,
										PolygonMessageKind.LAYER_NOT_PROJECTED, sp0Name
								).build()
				);
		...
		/*
		 * 2007/11/13: Fill in Diameter when it is available and not already computed.
		 *
		 */

		if (updateDiameter <= 0.0 && updateBasalArea >= 0.0 && updateTPH >= 0.0) {
			updateDiameter = computeDiameter(updateTPH, updateBasalArea);// V7Int_ComputeDiameter( *fTPH, *fBasalArea );
			logger.debug("Diameter computed from BA/TPH ({0},{1}): {2}", updateBasalArea, updateTPH, updateDiameter);
		}

		// update the yield record with the new values
		if (updateDiameter != yields.diameter() || updateSI != yields.siteIndex()
				|| updateDominantHeight != yields.dominantHeight() || updateTPH != yields.treesPerHectare()
				|| updateBasalArea != yields.basalArea125cm()) {

			logger.debug(
					"Updating layer yields with new values: SI={0}, DH={1}, TPH={2}, BA={3}, D={4}", updateSI,
					updateDominantHeight, updateTPH, updateBasalArea, updateDiameter
			);
			yields = new LayerYields(
					yields.bYieldsPredicted(), //
					yields.isDominantSp0(), //
					yields.sp0Name(), //
					yields.standYear(), //
					yields.speciesAge(), //
					yields.dominantHeight(), //
					yields.loreyHeight(), //
					updateSI, //
					updateDiameter, //
					updateTPH, //
					yields.wholeStemVolume(), //
					yields.closeUtilizationVolume(), //
					yields.cuVolumeLessDecay(), //
					yields.cuVolumeLessDecayWastage(), //
					yields.cuVolumeLessDecayWastageBreakage(), //
					updateBasalArea, // TODO which one should be updated
					updateBasalArea, // TODO which one should be updated
					yields.reportedStandPercent(), //
					yields.siteIndexCurve()
			);
		}
		return yields;
	}
