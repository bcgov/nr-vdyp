...
public class YieldTable implements Closeable {
	...
	private EntityGrowthDetails getProjectedLayerStandGrowthInfo(
		...
		var leadingSpeciesSp0 = layer.getSp0sByPercent().get(0);// layer.determineLeadingSp0(0);
	...
	private EntityGrowthDetails getProjectedLayerSpeciesGrowthInfo(
		...
		if (species.getTotalAge() != null && species.getSiteIndex() != null && species.getYearsToBreastHeight() != null
			...
			// Compute a species height at that new age.TODO this should use species total age...
	...
	/**
	 * copyPredictedSIAndHeightIfNotInput vdyp7intpolystats.c lines 4276 - 4303 2004/10/27: Based on e-mails from Cam,
	 * we now check if the returned species information has site information while the input data does not.
	 *
	 * If the returned species does have site information when the supplied species does not, this generally indicates
	 * that the leading species has become the dominant species although this may not always be the case.
	 *
	 * If we do get a new set of site values, these values will be based on the internally generated dominant height
	 * used by the VDYP7 model and does not reflect the dominant height reported back to the caller. Because of this, we
	 * need to fill in the input site information and then recompute the projected height. The reprojected height will
	 * come later in the process.
	 *
	 * 2004/10/30: Following on from the above comments and Cam's latest e-mail. We will no longer get the Extended Core
	 * Modules to fill in site information. Instead, we will rely on values generated by VDYP7.
	 *
	 * It seems IF we have projected yields we want to set this in the input data. The only reason I could see to do
	 * this is to PREVENT the supression of this same data later.
	 */
	void copyPredictedSIAndHeightIfNotInput(Stand stand, LayerYields layerYields) {
		if (stand != null && layerYields.bYieldsPredicted() && layerYields.dominantHeight() > 0
				&& stand.getSpeciesGroup().getDominantHeight() == null) {
			stand.getSpeciesGroup().setTotalAge(layerYields.speciesAge());
			stand.getSpeciesGroup().setDominantHeight(layerYields.dominantHeight());
			stand.getSpeciesGroup().setSiteIndex(layerYields.siteIndex());

			Species leadingSpecies = stand.getSpeciesByPercent().get(0);
			leadingSpecies.setTotalAge(layerYields.speciesAge());
			leadingSpecies.setDominantHeight(layerYields.dominantHeight());
			leadingSpecies.setSiteIndex(layerYields.siteIndex());
		}
	}
