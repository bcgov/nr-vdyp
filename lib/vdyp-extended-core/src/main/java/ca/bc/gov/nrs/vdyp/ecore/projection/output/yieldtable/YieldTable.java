...
public class YieldTable implements Closeable {
	...
	private LayerYields obtainStandYield(
		...
		/*
		 * vdyp7intpolystats.c lines 4276 - 4303 2004/10/27: Based on e-mails from Cam, we now check if the returned
		 * species information has site information while the input data does not.
		 *
		 * If the returned species does have site information when the supplied species does not, this generally
		 * indicates that the leading species has become the dominant species although this may not always be the case.
		 *
		 * If we do get a new set of site values, these values will be based on the internally generated dominant height
		 * used by the VDYP7 model and does not reflect the dominant height reported back to the caller. Because of
		 * this, we need to fill in the input site information and then recompute the projected height. The reprojected
		 * height will come later in the process.
		 *
		 * 2004/10/30: Following on from the above comments and Cam's latest e-mail. We will no longer get the Extended
		 * Core Modules to fill in site information. Instead, we will rely on values generated by VDYP7.
		 *
		...
		if (stand != null && layerYields.bYieldsPredicted() && layerYields.dominantHeight() > 0
				&& (stand.getSpeciesGroup().getDominantHeight() == null
						|| Double.isNaN(stand.getSpeciesGroup().getDominantHeight())
						|| stand.getSpeciesGroup().getDominantHeight() < 0)) {
