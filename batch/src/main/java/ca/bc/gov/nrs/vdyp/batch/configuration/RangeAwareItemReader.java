...
/**
 * ItemReader that reads only records within a specified line range. Used for
 * partitioned processing to ensure each
...
public class RangeAwareItemReader implements ItemReader<BatchRecord>, ItemStream {
    ...
    public BatchRecord read() throws Exception {
        ...
        while (true) {
            try {
                ...
                BatchRecord processedRecord = processVdypRecord(record);
     ...
     */
    private BatchRecord processVdypRecord(BatchRecord record) throws Exception {
        Long recordId = record.getId();

        if (recordId == null) {
            handleDataQualitySkip(record, "NULL_ID", "VDYP record has null ID");
            return null;
        }

        // Validate record data quality
        validateVdypRecordData(record);
     ...
     */
    private void validateVdypRecordData(BatchRecord record) throws Exception {
        Long recordId = record.getId();

        if (record.getData() == null || record.getData().trim().isEmpty()) {
            handleDataQualitySkip(record, "MISSING_VDYP_DATA", "VDYP data field is missing or empty");
        ...
        if (record.getPolygonId() == null || record.getPolygonId().trim().isEmpty()) {
            handleDataQualitySkip(record, "MISSING_POLYGON_ID", "Polygon ID is missing or empty");
        ...
        if (record.getLayerId() == null || record.getLayerId().trim().isEmpty()) {
            handleDataQualitySkip(record, "MISSING_LAYER_ID", "Layer ID is missing or empty");
     ...
     */
    private void handleDataQualitySkip(BatchRecord record, String skipReason, String description) {
        totalSkipsInReader.incrementAndGet();
        skipReasonCounts.computeIfAbsent(skipReason, k -> new AtomicLong(0)).incrementAndGet();

        if (record != null) {
            BatchSkipPolicy.cacheRecordData(record.getId(), record, Thread.currentThread().getName());
        }

        if (metricsCollector != null && jobExecutionId != null && record != null) {
            IllegalArgumentException dataQualityException = new IllegalArgumentException(description);
            Long lineNumber = record.getId() != null ? record.getId() + 1 : null;

            metricsCollector.recordSkip(
                    jobExecutionId, record.getId(), record, dataQualityException, partitionName, lineNumber);
        }

        logger.warn(
                "[{}] VDYP Data quality skip: {} for record ID {} - {}", partitionName, skipReason,
                record != null ? record.getId() : "unknown", description);
    }
