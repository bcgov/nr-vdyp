...
export const ProjectionApiAxiosParamCreator = function (
  ...
  return {
    ...
    /**
     * (Admin Only) Get all RUNNING projections across all users
     * @GET /api/v8/projection/all
     */
    getAllRunningProjections: async (
      options: AxiosRequestConfig = {},
    ): Promise<RequestArgs> => {
    ...
    /**
     * (Admin Only) Get the batch service's PVC storage status
     * @GET /api/v8/projection/storage-status
     */
    getStorageStatus: async (
      options: AxiosRequestConfig = {},
    ): Promise<RequestArgs> => {
      const localVarPath = `/api/v8/projection/storage-status`
      const localVarUrlObj = new URL(localVarPath, env.VITE_API_URL)
      let baseOptions
      if (configuration) {
        baseOptions = configuration.baseOptions
...
export const ProjectionApiFp = function (configuration?: Configuration) {
  return {
    ...
    async getAllRunningProjections(
      options?: AxiosRequestConfig,
    ): Promise<
      (
        axios?: AxiosInstance,
    ...
    > {
      const localVarAxiosArgs =
        await ProjectionApiAxiosParamCreator(configuration).getStorageStatus(
          options,
        )
      return (
        axios: AxiosInstance = globalAxios,
...
export class ProjectionApi extends BaseAPI {
   ...
   * (Admin Only) Get all RUNNING projections across all users
   */
  public async getAllRunningProjections(
    options?: AxiosRequestConfig,
  ): Promise<AxiosResponse<ProjectionModel[]>> {
    return ProjectionApiFp(this.configuration)
      .getAllRunningProjections(options)
