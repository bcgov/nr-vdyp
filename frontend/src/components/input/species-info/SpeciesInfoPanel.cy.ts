...
describe('SpeciesInfoPanel.vue', () => {
  ...
  const mountComponent = (overrides = {}) => {
    ...
    modelParameterStore.speciesList = [
      { species: 'PL', percent: '50' },
      { species: 'AC', percent: '50' },
    ]
  ...
  it('renders correctly with initial state', () => {
    ...
    // Verify that SpeciesListInput is rendered
    // cy.get('input').should('have.value', 'PL')
    // cy.get('input').should('have.value', '50')

    // Verify that Total Species Percent text field is rendered and has the expected value
    // cy.get('input[aria-label="Total Species Percent"]')
    //   .should('exist')
    //   .and('have.value', '100')
    //   .and('be.disabled')
