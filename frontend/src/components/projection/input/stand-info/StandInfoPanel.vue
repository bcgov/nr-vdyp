      ...
      <v-expansion-panel hide-actions>
        <v-expansion-panel-title>
          <v-row no-gutters class="expander-header">
            ...
            <v-col>
              <span class="text-h6">{{ CONSTANTS.MANUAL_INPUT_PANEL_LABEL.STAND_INFO }}</span>
            </v-col>
            <PanelEditControl
...
const isHeaderEditActive = computed(() => {
  ...
  if (status === CONSTANTS.PROJECTION_STATUS.RUNNING || status === CONSTANTS.PROJECTION_STATUS.STUCK || status === CONSTANTS.PROJECTION_STATUS.READY) return false
  return isConfirmed.value && !modelParameterStore.panelState[panelName].editable
})

const editTooltipText = computed(() => {
  const status = appStore.currentProjectionStatus
  if (status === CONSTANTS.PROJECTION_STATUS.RUNNING || status === CONSTANTS.PROJECTION_STATUS.STUCK || status === CONSTANTS.PROJECTION_STATUS.READY) {
