...
class Hcsv_Vdyp7_Comparison_Test {
	...
	@Test
	void test1080() throws IOException, ResourceParseException, URISyntaxException, CsvException {

		logger.info("Starting test1080");

		Map<String, List<String>> paramMap = new HashMap<>();
		var parameters = new Parameters();
		try (
				InputStream paramStream = MainTest.class.getResourceAsStream("vdyp-1180/input/parms.txt");
				InputStreamReader reader = new InputStreamReader(paramStream);
				BufferedReader bufReader = new BufferedReader(reader); var lines = bufReader.lines();
		) {
			ParamsReader.parseParameters(paramMap, lines);
		...
		try (
				InputStream polyStream = MainTest.class.getResourceAsStream("vdyp-1180/input/VDYP7_INPUT_POLY.csv");
				InputStream layerStream = MainTest.class.getResourceAsStream("vdyp-1180/input/VDYP7_INPUT_LAYER.csv");
		) {
			...
			try (
					CSVReader vdyp7Stream = new CSVReader(
							new InputStreamReader(
									MainTest.class.getResourceAsStream("vdyp-1180/output/VDYP7YieldTable.csv")
							)
					); CSVReader vdyp8Stream = new CSVReader(new StringReader(vdyp8YieldTableContent));
			) {
				Pattern ignorePattern = Pattern.compile("");
