...
*/
// private static final int SI_ERR_LT13      = -1; Unused
// private static final int SI_ERR_GI_MIN    = -2; Unused
// private static final int SI_ERR_GI_MAX    = -3; Unused
private static final int SI_ERR_NO_ANS    = -4;
private static final int SI_ERR_CURVE     = -5;
// private static final int SI_ERR_CLASS     = -6; Unused
// private static final int SI_ERR_FIZ       = -7; Unused
// private static final int SI_ERR_CODE      = -8; Unused
// private static final int SI_ERR_GI_TOT    = -9; Unused
private static final int SI_ERR_SPEC      = -10;
// private static final int SI_ERR_AGE_TYPE  = -11; Unused
...
//BOOL APIENTRY DllMain still to be implemented
 ...
 * Enumerate available species
 -----------------------------------------------------------------------------*/
public static short Sindex_NextSpecies (short sp_index){ 
 ...
 *    Species code string takes the form "Xx" or "Xxx", such as "Sw" or "Fdc".
 -----------------------------------------------------------------------------*/
public static String Sindex_SpecCode(short sp_index){
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
            return null;
    }
 ...
 *    Species name string examples: "Coastal Douglas-fir", "Sitka Spruce".
 -----------------------------------------------------------------------------*/
public static String Sindex_SpecName(short sp_index){
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
        return null;
    }
 ...
 *       100: common species in BC (0 means uncommon)
 ----------------------------------------------------------------------------- */
public static short Sindex_SpecUse (short sp_index){
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
        return SI_ERR_SPEC;
    }
 ...
 *    SI_ERR_NO_ANS   no curves defined for this species
 ----------------------------------------------------------------------------- */
public static short Sindex_DefCurve(short sp_index) {
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
      return SI_ERR_SPEC;
    }
 ...
 *    SI_ERR_NO_ANS   no GI equations defined for this species.
 ----------------------------------------------------------------------------- */
public static short Sindex_DefGICurve (short sp_index) {
  if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
    return SI_ERR_SPEC;
  }
 ...
 *    SI_ERR_NO_ANS   no curves defined for this species
 ----------------------------------------------------------------------------- */
public static short Sindex_DefCurveEst (short sp_index, short estab) {
  if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
    return SI_ERR_SPEC;
  }
 ...
 *    No assumption should be made about the ordering of the curves.
 -----------------------------------------------------------------------------*/
public static short Sindex_FirstCurve (short sp_index) {
  if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
    return SI_ERR_SPEC;
  }
 ...
 *    No assumption should be made about the ordering of the curves.
 -----------------------------------------------------------------------------*/
public static short Sindex_NextCurve (
  short sp_index,
  short cu_index)
  {
  if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
 ...
 *  Return Value
 *  ------------
 *    Pointer to string containing curve author and date.
 ...
 *      1000: si = fn (ht, age) growth intercept
 -----------------------------------------------------------------------------*/
public static short Sindex_CurveUse (short cu_index) {
  if (cu_index >= 0 && cu_index < SI_MAX_CURVES){
 ...
 *    Integer age type.
 *      Must be one of:
 *        SI_AT_TOTAL    The age is the total age of the stand in years since
 *                planting.
 *
 *        SI_AT_BREAST  The age indicates the number of years since the stand
 *                reached breast height.
 *
 *    Floating point height.
 *      The height of the species in metres.
 *
 *    Integer estimate type.
 *      Must be one of:
 ...
 *
 *    Floating point site index. (computed)
 *      This value is computed from the other parameters.
...
public static short Sindex_HtAgeToSI(
    ...
    double height, 
    short estType, 
    double[] site) //Here the original uses a pointer. I have replaced all pointers with arrays. This may cause issues in the later call of height_to_index
    {
 ...
 *      The site index value of the stand.
 *
 *    Floating point years to breast height.
 *      The number of years it takes the stand to reach breast height.
 *
 *    Floating point height. (computed)
 *      This value is computed from the other parameters.
 *      If an error condition occurs, the height is set to the
 *        same as the return value.
 *
 *  Return Value
 *  ------------
 ...
 *      The number of years it takes the stand to reach breast height.
 *
 *    Floating point seedling age.
 *      Average age(years) of planted seedling stock (0 if not known).
 *
 *    Floating point seedling height.
 *      Average height(m) of planted seedling stock (0 if not known).
 *
 *    Floating point height. (computed)
 *      This value is computed from the other parameters.
 *      If an error condition occurs, the height is set to the
 ...
 *      The site index value of the stand.
 *
 *    Floating point years to breast height. (computed)
 *      This value is computed from the other parameters.
 *      If an error condition occurs, the y2bh is set to the
 *        same as the return value.
 *
 *  Return Value
 *  ------------
 ...
 *    SI_ERR_LT13     if site index <= 1.3
 -----------------------------------------------------------------------------*/
public static short Sindex_Y2BH05 (short curve, double siteIndex, double[] y2bh){
 ...
 *    SI_ERR_LT13     if site index <= 1.3
 -----------------------------------------------------------------------------*/
public static short Sindex_Y2BH (short curve, double siteIndex, double[] y2bh)
  {
 ...
 *  Parameters
 *  ----------
 *    Integer source species index.
 *    Floating point source species site index.
 *    Integer target species index.
 ...
 *  ----------
 *    Integer species index.
 *    Character site class ('G', 'M', 'P', 'L').
 *    Character FIZ code (A,B,C)=coast, (D,E,F,G,H,I,J,K,L)=interior.
 *    Floating point site index. (computed)
 *
 *  Return Value
 *  ------------
 ...
 *      is ignored.
 -----------------------------------------------------------------------------*/
public static short Sindex_SpecMap (String sc){
 ...
 *      code "FD".
 -----------------------------------------------------------------------------*/
public static short Sindex_SpecRemap (String sc, char fiz){
 ...
 *  Return Value
 *  ------------
 *    Pointer to string containing publication citation.
 *    If input parameter is not a valid curve index, the return is the
 *    null pointer.
 -----------------------------------------------------------------------------*/
public static String Sindex_CurveSource (short cu_index){
  if (cu_index < 0 || cu_index >= SI_MAX_CURVES){
    return null;
  }
 ...
 *  Return Value
 *  ------------
 *    Pointer to string containing notes on use of curve.
 *    If input parameter is not a valid curve index, the return is the
 *    null pointer.
 -----------------------------------------------------------------------------*/
public static String Sindex_CurveNotes (short cu_index)
  {
  if (cu_index < 0 || cu_index >= SI_MAX_CURVES){
    return null;
  }
 ...
 *  ----------
 *    Integer curve index.
 *    Floating point source age.
 *    Integer type of source age (SI_AT_BREAST or SI_AT_TOTAL).
 *    Floating point years to breast height.
 *    Floating point target age. (computed)
 *    Integer type of target age (SI_AT_BREAST or SI_AT_TOTAL).
 *
 *  Return Value
 *  ------------
 *    0, or an error code under the following conditions:
 ...
 *  -------
 -----------------------------------------------------------------------------*/
public static short Sindex_CurveToSpecies (short cu_index){
  if (cu_index < 0 || cu_index >= SI_MAX_CURVES){
    return SI_ERR_CURVE;
  }
