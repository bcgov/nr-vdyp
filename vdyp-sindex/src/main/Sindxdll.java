...
//private static final int SI_ZH_END      = SI_ERR_NO_ANS; Unused
    ...
    private static final String[][] si_curve_notes = {
        ...
        {
        // The following is conditional code for #ifdef HOOP but I have removed the condition. From what I can tell
...
private static final int[] si_sclist_start = {
  SI_A_START,
  SI_ABAL_START,
  SI_ABCO_START,
  SI_AC_START,
 ...
 *
 * @remarks The format of the number is always in the form of:
 *           Mmm
 ...
 *
 *          Minor release changes may include:
 *          - Addition of a function.
 *          - Changed return values (e.g., error messages).
 ...
 */
public static short Sindex_VersionNumber() {
    return 151;
}
...
*/
public static short Sindex_FirstSpecies(){   
 ...
 * @throws IllegalArgumentException if the input parameter is not a valid species index.
 * @throws NoSuchElementException if the input parameter is the last defined species index.
 *
 * @remarks No assumption should be made about the ordering of the species.
 */
public static short Sindex_NextSpecies(short sp_index) throws IllegalArgumentException, NoSuchElementException {
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES) {
        ...
        throw new NoSuchElementException("Input parameter is the last defined species index: " + sp_index);
    }
...
*/
public static String Sindex_SpecCode(short sp_index) throws IllegalArgumentException{
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
        throw new IllegalArgumentException("Input parameter is not a valid species index: " + sp_index);
    }
...
*/
public static String Sindex_SpecName(short sp_index) throws IllegalArgumentException{
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
       throw new IllegalArgumentException("Input parameter is not a valid species index: " + sp_index);
    }
 ...
 *    
 * @remarks Code bits are set as follows:
 *                1: BC coast
 *                10: BC interior
 *                100: common species in BC (0 means uncommon)
*/
public static short Sindex_SpecUse (short sp_index) throws IllegalArgumentException{
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
       throw new IllegalArgumentException("Input parameter is not a valid species index: " + sp_index);
    }
 ...
 * @throws NoSuchElementException if the input parameter is the last defined species index.
 */
public static short Sindex_DefCurve(short sp_index) throws IllegalArgumentException, NoSuchElementException {
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES) {
        throw new IllegalArgumentException("Input parameter is not a valid species index: " + sp_index);
    } else if (sp_index == SI_SPEC_END) {
        throw new NoSuchElementException("Input parameter is the last defined species index: " + sp_index);
 ...
 * 
 * @throws IllegalArgumentException if the input parameter is not a valid species index.
 * @throws NoSuchElementException No GI equations defined for this species.
 * */
public static short Sindex_DefGICurve (short sp_index) throws IllegalArgumentException, NoSuchElementException {
    if (sp_index < 0 || sp_index >= SI_MAX_SPECIES) {
        throw new IllegalArgumentException("Input parameter is not a valid species index: " + sp_index);
    }
...
throw new NoSuchElementException("No GI equations defined for this species: " + sp_index);
}
 ...
 * @return Integer curve index, for use in other Sindex functions.
 * 
 * @throws IllegalArgumentException if the input parameter is not a valid species index or establishment type.
 * @throws NoSuchElementException if no curves defined for this species.
 * 
 * @remarks Orginally said to return SI_ERR_NO_ANS if no curves are defined for the species. I've added an additional check to see if it
 * would return this and thrown the NoSuchElementException there instead.
 */
public static short Sindex_DefCurveEst (short sp_index, short estab) throws IllegalArgumentException, NoSuchElementException {
  if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
    throw new IllegalArgumentException("Input parameter is not a valid species index: " + sp_index);
  }
  ...
  if (sp_index == SI_SPEC_SW) {
    switch (estab) {
      ...
      //  break; This is unreachable not sure why it was included in the C program
      default:
        throw new IllegalArgumentException("Input parameter is not a valid establishment type: " + estab);
      }
  }else if (si_curve_default[sp_index] == SI_ERR_NO_ANS){
    throw new NoSuchElementException("No curves defined for this species: " + sp_index);
  }else {
    return (short) (si_curve_default[sp_index]);
  }
}
 ...
 * would return this and thrown the NoSuchElementException there instead.
*/
public static short Sindex_FirstCurve (short sp_index) throws IllegalArgumentException, NoSuchElementException {
  if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
    throw new IllegalArgumentException("Input parameter is not a valid species index: " + sp_index);
 ...
 * @return Integer curve index, for use in other Sindex functions.
 *
 * @throws IllegalArgumentException if input species is not a valid species index or if input curve is not a valid curve index for this species.
 * @throws NoSuchElementException   if input parameter is last defined index for this species.
 *
 * @remarks No assumption should be made about the ordering of the curves.
 */
public static short Sindex_NextCurve  (short sp_index, short cu_index) throws IllegalArgumentException, NoSuchElementException{
  if (sp_index < 0 || sp_index >= SI_MAX_SPECIES){
    throw new IllegalArgumentException("Input species is not a valid species index: " + sp_index);
  }
  
  if (cu_index < 0 || cu_index >= SI_MAX_CURVES){
    throw new IllegalArgumentException("Input curve is not a valid curve index for this species: " + cu_index);
    }
  ...
  switch (cu_index){
    ...
    case SI_PLI_THROWNIGH: cu_index = SI_PLI_NIGHTA98; break;
  ...
  if(cu_index == SI_ERR_NO_ANS){
    throw new NoSuchElementException("Input parameter is last defined index for this species: " + sp_index);
  } else{
    return cu_index;
  }
}
...
*/
public static String Sindex_CurveName(short cu_index) {
    if (cu_index >= 0 && cu_index < SI_MAX_CURVES){
        return si_curve_name[cu_index];
    }

    throw new IllegalArgumentException("Input parameter is not a valid curve index: " + cu_index);
}
 ...
 * @return Integer code.
 * 
 * @throws IllegalArgumentException if input curve is not a valid curve index.
 *
 * @remarks Code bits are set as follows:
 *             0001: ht = fn (si, age)
 *             0010: si = fn (ht, age)
 ...
 */
public static short Sindex_CurveUse (short cu_index) throws IllegalArgumentException {
  if (cu_index >= 0 && cu_index < SI_MAX_CURVES){
    return (short) (si_curve_types[cu_index]);
  }
  throw new IllegalArgumentException("If input curve is not a valid curve index: " + cu_index);
}
 ...
 * @param age      Floating point age. The age of the trees indicated by the curve selection. The 
 *                 interpretation of this age is modified by the 'ageType' parameter.
 * @param ageType  Integer age type. 
 *                 Must be one of:
 *                      SI_AT_TOTAL    The age is the total age of the stand in years since planting.
 *                      SI_AT_BREAST   The age indicates the number of years since the stand reached breast height.
 * @param height   Floating point height. The height of the species in metres.
 * @param estType  Integer estimate type.
 *                 Must be one of:
 *                    SI_EST_DIRECT    Compute the site index based on direct equations if available.  If the equations are not 
 *                                     available, then automatically fall to the SI_EST_ITERATE method.
 *                    SI_EST_ITERATE    Compute the site index based on an iterative method which converges on the true site index.
 * @param site     Floating point site index. (computed) This value is computed from the other parameters.
 *                 If an error condition occurs, the site index is set to the same as the return value.
 * @return  0 or an exception
 * 
 * @throws IllegalArgumentException if input curve is not a valid curve index
 * @throws NoSuchElementException if computed SI > 999
 * @throws IllegalArgumentException if bhage < 0.5
 * @throws IllegalArgumentException if bhage > GI range
 * @throws IllegalArgumentException if total age and GI curve
 */
public static short Sindex_HtAgeToSI(
    short curve, 
    double age, 
    short ageType, 
    double height, 
    short estType, 
    double[] site)  throws IllegalArgumentException, NoSuchElementException
    ...
    if (site[0] < 0){
      switch ((short) site[0]){
        case SI_ERR_CURVE:
          throw new IllegalArgumentException("Input curve is not a valid curve index.");
        case SI_ERR_GI_MIN:
          throw new IllegalArgumentException("bhage is less than 0.5");
        case SI_ERR_GI_MAX:
          throw new IllegalArgumentException("bhage is greater than GI range");
        case SI_ERR_NO_ANS:
          throw new NoSuchElementException("Computed site index is greater than 999.");
        case SI_ERR_GI_TOT:
          throw new IllegalArgumentException("Total age and GI curve");
      }   
    }
 ...
 * @param age         Floating point age. (computed) This value is computed from the other parameters.
 *                    If an error condition occurs, the age is set to the same as the return value.
 * @return 0, or an exception 
 * 
 * @throws IllegalArgumentException if input curve is not a valid curve index
 * @throws IllegalArgumentException if bhage < 0.5
 ...
 * @throws NoSuchElementException if computed SI > 999
 * @throws IllegalArgumentException if total age and GI curve
 * @throws IllegalArgumentException  if site index <= 1.3
 */
public static short Sindex_HtSIToAge (
  short      curve,
  double    height,
  short      ageType,
  double    siteIndex,
  double     y2bh,
  double[]   age)  throws IllegalArgumentException, NoSuchElementException
  {
  ...
  if (age[0] < 0){
    switch ((short) age[0]){
      ...
      case SI_ERR_GI_TOT:
        throw new IllegalArgumentException("Total age and GI curve");
      case SI_ERR_LT13:
        throw new IllegalArgumentException("Site index is less than or equal to 1.3");
    }  
  }
 ...
 *                      SI_AT_BREAST  The age indicates the number of years since the stand reached breast height.
 * @param siteIndex   Floating point site index. The site index value of the stand.
 * @param y2bh        Floating point years to breast height. The number of years it takes the stand to reach breast height.
 * @param height      Floating point height. (computed) This value is computed from the other parameters.
 *                    If an error condition occurs, the height is set to the same as the return value.
 * 
 * @return 0, or an exception
 * 
 * @throws IllegalArgumentException if input curve is not a valid curve index
 ...
 * @throws IllegalArgumentException  if site index <= 1.3
 */
public static short Sindex_AgeSIToHt (
  short     curve,
  double    age,
  short     ageType,
  double    siteIndex,
  double    y2bh,
  double[]  height)  throws IllegalArgumentException, NoSuchElementException
  {
...
/**
 *  Sindex_AgeSIToHtSmooth
 *  ======================
 *    Converts an Age and Site Index to a Height for a particular Site Index
 *    Curve.  This includes a smoothing equation centered at breast-height
 *    age 0.  Also, user can specify seedling age and height.
 *
 * @param curve         Integer curve index. The particular site index curve to project the height and age along.
 * @param age           Floating point age. The age of the trees indicated by the curve selection. The
 *                      interpretation of this age is modified by the 'ageType' parameter.
 ...
 * @throws IllegalArgumentException if site index <= 1.3
 */
public static short Sindex_AgeSIToHtSmooth(
  short     curve,
  double    age,
  short     ageType,
  double    siteIndex,
 ...
 * @param curve       Integer curve index. The particular site index curve to project the height and age along.
 * @param siteIndex   Floating point site index. The site index value of the stand.
 * @param y2bh        Floating point years to breast height. (computed) This value is computed from the other parameters.
 *                    If an error condition occurs, the y2bh is set to the same as the return value.
 *
 * @return 0, or an exception 
 *    
 * @throws IllegalArgumentException  input curve is not a valid curve index, if GI curve, or if site index <= 1.3    
 */
public static short Sindex_Y2BH05 (short curve, double siteIndex, double[] y2bh) throws IllegalArgumentException{
  ...
  if (y2bh[0] < 0){
    switch((short) y2bh[0]){
      case SI_ERR_CURVE:
        throw new IllegalArgumentException("Input curve is not a valid curve index");
      case SI_ERR_GI_TOT:
        throw new IllegalArgumentException("GI curve");
      case SI_ERR_LT13:
        throw new IllegalArgumentException("Site index is less than or equal to 1.3");
    }
 ...
 * @throws IllegalArgumentException  input curve is not a valid curve index, if GI curve, or if site index <= 1.3    
 */
public static short Sindex_Y2BH (short curve, double siteIndex, double[] y2bh) throws IllegalArgumentException
  {
 ...
 * @return 0, or an exception
 * 
 * @throws IllegalArgumentException source or target species index is not valid.
 * @throws NoSuchElementException  there is no conversion defined.
 */
public static short Sindex_SIToSI (
  short      sp_index1,
  double     site,
  short      sp_index2,
  double[]   site2) throws IllegalArgumentException, NoSuchElementException
  {
  short i;
  ...
  if (sp_index1 < 0 || sp_index1 >= SI_MAX_SPECIES)
    {
    site2[0] = SI_ERR_SPEC;
    throw new IllegalArgumentException("Source or target species index is not valid" + sp_index1);
    }
  
  if (sp_index2 < 0 || sp_index2 >= SI_MAX_SPECIES)
    {
    site2[0] = SI_ERR_SPEC;
    throw new IllegalArgumentException("Source or target species index is not valid" + sp_index2);
    }
  ...
  throw new NoSuchElementException("There is no conversion defined");
  }
 ...
 * @return 0, or an exception
 *    
 * @throws IllegalArgumentException source species index is not valid, or no conversion, if site class is unknown, orif FIZ code is unknown
 */
public static short Sindex_SCToSI (
  short     sp_index,
  char      sitecl,
  char      fiz,
  double[]    site)
  ...
  if (site[0] < 0){
    switch((short) site[0]){
      case SI_ERR_SPEC:
        throw new IllegalArgumentException("Source species index is not valid, or no conversion");
      case SI_ERR_CLASS:
        throw new IllegalArgumentException("Site class is unknown");
      case SI_ERR_FIZ:
        throw new IllegalArgumentException("FIZ code is unknown");
    }
  }
 ...
 * 
 * @throws IllegalArgumentException if species code is unknown    
 *
 * @remarks Species code string can be 1, 2, or 3 letters; upper/lower case is ignored.
 */
public static short Sindex_SpecMap (String sc) throws IllegalArgumentException{
 ...
 * @return Species index.
 * 
 * @throws IllegalArgumentException if species code is unknown or if FIZ code is unknown
 *
 * @remark Species code string can be 1, 2, or 3 letters; upper/lower case is ignored.  FIZ 
 *         is only used where needed, such as for species code "FD".
 */
public static short Sindex_SpecRemap (String sc, char fiz) throws IllegalArgumentException{
  ...
  if(speciesIndex == SI_ERR_FIZ){
    throw new IllegalArgumentException("Character FIZ code (A,B,C)=coast, (D,E,F,G,H,I,J,K,L)=interior" + fiz);
  }
 ...
 * @throws IllegalArgumentException if input parameter is not a valid curve index
 */
public static String Sindex_CurveSource (short cu_index) throws IllegalArgumentException{
  if (cu_index < 0 || cu_index >= SI_MAX_CURVES){
 ...
 * @throws IllegalArgumentException if input parameter is not a valid curve index
 */
public static String Sindex_CurveNotes (short cu_index) throws IllegalArgumentException
  {
  if (cu_index < 0 || cu_index >= SI_MAX_CURVES){
    throw new IllegalArgumentException("Invalid curve index: " + cu_index);
  }
 ...
 * @return 0 or an exception
 *    
 * @throws  IllegalArgumentException input curve is not a valid curve index for this species or unknown age type.
 */
public static short Sindex_AgeToAge (
  short     cu_index,
  double    age1,
  short     age_type1,
  double    y2bh,
  double[]  age2,
  short     age_type2) throws IllegalArgumentException
  {
  age2[0] = age_to_age (cu_index, age1, age_type1, age_type2, y2bh);
  if (age2[0] < 0){
    switch((short) age2[0]){
      case SI_ERR_CURVE:
        throw new  IllegalArgumentException("Input curve is not a valid curve index for this species");
 ...
 *    
 * @throws  IllegalArgumentException if input curve is not a valid curve index for any species.
 */
public static short Sindex_CurveToSpecies (short cu_index) throws  IllegalArgumentException {
  if (cu_index < 0 || cu_index >= SI_MAX_CURVES){
    throw new IllegalArgumentException("Input curve is not a valid curve index for any species");
  }
